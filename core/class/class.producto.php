<?php 
	...
	*/
	class Producto extends DB
	{
		...
		public function setColours($array = array(),$id = null){

			if(!is_null($id)){
				$sql = self::PRODUCTO_CREATE_COLOURS;
				$long_sql = "";
				foreach ($array as $k => $v) {
					$tmp = str_replace(":prod",$id,$sql);
					$tmp = str_replace(":colour",$k,$tmp);
					$tmp = str_replace(":cnt",$v,$tmp);
					$long_sql .= $tmp;
				}

				return $this->exec($long_sql);
			}
		}

		public function setSizeClothes($array = array(), $id = null){
			if(!is_null($id)){
				$sql = self::PRODUCTO_CREATE_SIZECLOTHES;
				$long_sql = "";
				foreach ($array as $k => $v) {
					$tmp = str_replace(":prod",$id,$sql);
					$tmp = str_replace(":sizeid",$k,$tmp);
					$tmp = str_replace(":cnt",$v,$tmp);
					$long_sql .= $tmp;
				}
				echo $long_sql;
		...
		public function setColourSizeClothes($array = array(), $id = null){
			if(!is_null($id)){
				$base_sql = "INSERT INTO colores_talles  (id_producto,id_color,id_talle,cantidad) VALUES ";
				$i = 0;
				foreach ($array as $kprod => $vprod) {
					$colour = $vprod['color'];
					foreach ($vprod['talle'] as $kSize => $vSize) {
						if($i == 0){
							$base_sql .= "(".$id.",".$colour.",".$kSize.",".(int)$vSize.")";
							$i++;
						}else{
							$base_sql .= ",(".$id.",".$colour.",".$kSize.",".(int)$vSize.")";
						}
