	...
	*/
	...
	{	
		...
		*/
		...
		/**
		 * @todo Traigo todos los pedidos con tiempo mayor a 24hs
		 */

		public function getExpiredShoppingCard($expiration = 24){
			$sel = $this->prepare(self::MAXCOMPRA_GETEXPIRED);
			$sel->bindParam(":expiration", $expiration, PDO::PARAM_INT);
			$sel->execute();
		...
		*/
		private function setInitsData(){
			$usuario = ( isset($_SESSION['MM_IdUsuario']) ? $_SESSION['MM_IdUsuario'] : '');
			$this->user = (!empty($usuario) && !is_null($usuario) ? $usuario : '');
