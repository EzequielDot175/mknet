...
<textarea name="message" class="campo-prod-detalle valid"></textarea>
...
<select class="campo-prod valid"  name="destacado" >
...
<script>
	$(function() {
		$('#item-categorias2').trigger('change');
		$(document).on('click', '.newColor', function(event) {
			event.preventDefault();
			var cat = $('#item-categorias2').val();
			console.log(cat);
			//var id = window.location.search.match(/id=.*[0-9].&/ig);
				//id = parseInt(id[0].replace(/id=/ig,'').replace(/&/ig,''));
			$.get('./talles_by_cat.php?idcategoria='+cat+'&idproducto=0&action=add', function(data) {
				console.log(data);
		...
		$(document).on('click', '.removeColor', function(event) {
			...
			if (confirm('¿Esta seguro de borrar este color y talles?')) {
				if (color != null) {
					$.get('./talles_by_cat.php?idcategoria='+cat+'&idproducto=0&action=delete&color='+color, function(data) {
						console.log(data);
					});
				};
