...
// ini_set('display_error', 'on');
	class tallesColores extends PDO
	{
		...
		public function productExist($user,$prod,$size,$colour){
			$sql = "SELECT IF( COUNT(intContador) > 0 , 1, 0 ) as exist FROM carrito WHERE idUsuario = ".$user." AND idProducto = ".$prod." AND talle = ".$size." AND color = ".$colour;
			$sel = $this->query($sql)->fetch(PDO::FETCH_OBJ)->exist;
			return ($sel == 1 ? true : false);
			//return $sql;
		}

		public function updateShoppingCartItem($count,$user,$prod,$size,$colour){
			$sql = "UPDATE carrito SET intCantidad = intCantidad +".$count." WHERE idUsuario = ".$user." AND idProducto = ".$prod." AND talle = ".$size." AND color = ".$colour;
